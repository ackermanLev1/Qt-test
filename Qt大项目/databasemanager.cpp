...
DataBaseManager::DataBaseManager() {
    ...
    db.setDatabaseName("./bookLendingSystem.db"); // 数据库文件存储在项目根目录
}
...
bool DataBaseManager::initDataBase() {
    ...
    QString createBorrowTable = R"(
        CREATE TABLE IF NOT EXISTS borrow (
            borrowId INT PRIMARY KEY AUTOINCREMENT,
    ...
    if (!query.next()) {
        ...
        if (!query.exec(insertAdmin)) {
            qDebug() << "插入管理员数据失败：" << query.lastError().text();
        }
    }
    ...
    if (query.next() && query.value(0).toInt() < 3) {
        ...
        for (auto& reader : readers) {
            QString insertReader = "INSERT INTO user_Reader (readerId, username, pwd, name, gender, phone, registerTime) VALUES " + reader;
            query.exec(insertReader);
        }
    }
    ...
    if (query.next() && query.value(0).toInt() < 20) {
        QString books[] = {
            "('" + generateBookId() + "', 'C++ Primer', 'Stanley B. Lippman', '人民邮电出版社', '2021-01-15', '编程', 10)",
            "('" + generateBookId() + "', 'Qt 5.15 开发实战', '霍亚飞', '机械工业出版社', '2022-03-20', '编程', 8)",
            "('" + generateBookId() + "', '数据结构与算法分析', 'Mark Allen Weiss', '清华大学出版社', '2020-11-05', '计算机', 5)",
            "('" + generateBookId() + "', '深入理解计算机系统', 'Randal E. Bryant', '机械工业出版社', '2019-08-10', '计算机', 7)",
            "('" + generateBookId() + "', 'Python编程：从入门到实践', 'Eric Matthes', '人民邮电出版社', '2021-05-18', '编程', 12)",
            "('" + generateBookId() + "', '机器学习实战', 'Peter Harrington', '人民邮电出版社', '2020-07-22', '人工智能', 6)",
            "('" + generateBookId() + "', '深度学习', 'Ian Goodfellow', '人民邮电出版社', '2018-01-30', '人工智能', 4)",
            "('" + generateBookId() + "', '数据库系统概念', 'Abraham Silberschatz', '机械工业出版社', '2019-03-15', '数据库', 9)",
            "('" + generateBookId() + "', '操作系统导论', 'Remzi H. Arpaci-Dusseau', '人民邮电出版社', '2021-09-01', '计算机', 3)",
            "('" + generateBookId() + "', '网络编程实战', '游双', '机械工业出版社', '2022-01-10', '编程', 8)",
            "('" + generateBookId() + "', '算法图解', 'Aditya Bhargava', '人民邮电出版社', '2019-06-25', '算法', 11)",
            "('" + generateBookId() + "', '设计模式：可复用面向对象软件的基础', 'Erich Gamma', '机械工业出版社', '2020-02-18', '编程', 5)",
            "('" + generateBookId() + "', '计算机网络', 'Andrew S. Tanenbaum', '清华大学出版社', '2018-11-08', '计算机', 6)",
            "('" + generateBookId() + "', 'Java编程思想', 'Bruce Eckel', '机械工业出版社', '2019-05-20', '编程', 4)",
            "('" + generateBookId() + "', '机器学习基石', 'Hastie Trevor', '清华大学出版社', '2021-03-12', '人工智能', 7)",
            "('" + generateBookId() + "', '数据分析实战', 'Peter Bruce', '人民邮电出版社', '2020-08-30', '数据处理', 9)",
            "('" + generateBookId() + "', 'Python数据分析', 'Wes McKinney', '人民邮电出版社', '2019-07-15', '数据处理', 10)",
            "('" + generateBookId() + "', 'Web开发实战', 'Matt Frisbie', '机械工业出版社', '2022-04-05', '编程', 6)",
            "('" + generateBookId() + "', '移动应用开发', 'Y. Daniel Liang', '清华大学出版社', '2021-01-22', '编程', 3)",
            "('" + generateBookId() + "', '云计算导论', 'Michael Miller', '人民邮电出版社', '2020-09-18', '计算机', 8)"
        };
        for (auto& book : books) {
            QString insertBook = "INSERT INTO book (bookId, name, author, publisher, publishDate, category, stock) VALUES " + book;
...
int DataBaseManager::loginVerify(const QString& username, const QString& pwd, QString& userId) {
    QSqlQuery query;
    // 先查管理员
    ...
    if (query.exec() && query.next()) {
        userId = query.value("adminId").toString();
        return 2; // 管理员
    }
    // 再查读者
    ...
    query.addBindValue(pwd);
    if (query.exec() && query.next()) {
        userId = query.value("readerId").toString();
        return 1; // 读者
    }
    return 0; // 登录失败
}

bool DataBaseManager::readerRegister(const QString& username, const QString& pwd, const QString& name, const QString& gender, const QString& phone) {
    QSqlQuery query;
    // 检查用户名是否已存在
    ...
    // 注册读者
    QString readerId = generateReaderId();
...
QSqlQueryModel* DataBaseManager::getBookList(const QString& searchKey) {
    ...
    if (!searchKey.isEmpty()) {
        sql += " WHERE bookId LIKE '%" + searchKey + "%' OR name LIKE '%" + searchKey + "%'";
    }
    model->setQuery(sql);
    // 设置列名
...
QSqlQueryModel* DataBaseManager::getReaderList(const QString& searchKey) {
    ...
    if (!searchKey.isEmpty()) {
        sql += " WHERE readerId LIKE '%" + searchKey + "%' OR name LIKE '%" + searchKey + "%'";
    }
    model->setQuery(sql);
...
QString DataBaseManager::checkOverdue(const QString& readerId) {
    ...
    while (query.next()) {
        ...
        if (QDateTime::currentDateTime() > dueDt) {
            overdueBooks += query.value("name").toString() + "（逾期" + QString::number(dueDt.daysTo(QDateTime::currentDateTime())) + "天）\n";
        }
    }
    return overdueBooks;
}
...
QString DataBaseManager::generateBookId() {
    ...
    QString bookId = "BK";
    for (int i = 0; i < 6; ++i) {
        bookId += chars.at(QRandomGenerator::global()->bounded(chars.length()));
    }
    return bookId;
}
...
QString DataBaseManager::generateReaderId() {
    ...
    QString readerId;
    // 1位数字
    readerId += digits.at(QRandomGenerator::global()->bounded(digits.length()));
    // 4位字母
    for (int i = 0; i < 4; ++i) {
        readerId += letters.at(QRandomGenerator::global()->bounded(letters.length()));
    }
    return readerId;
}
